...
private int auto_update=0;
  ...
  public void teleopPeriodic() {
    ...
    //  These functions are actually intended for autonomous
    //  but are flawed in that they require several flags
    //  to be set to prevent one operation from running into
    //  another.  Probably best to use the joystick within this
    //  block exclusively.  But they can be tested here one at
    //  a time.  Better functions are in the thread file.  They
    //  use while( ...) loops and do not require initialization in that
    //  each operation is concluded before the next one starts.
    //  Use of while( ... ) loops in the main thread may conflict with the
    //  watchdog.
